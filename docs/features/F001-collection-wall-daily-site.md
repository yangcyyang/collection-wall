...
宪宪验收 c9cf347（拆掉写死条数的测试）时追加两条测试技术债（2026-07-26，进 Phase 2+ backlog）：
| R-6 | P4 | `test/featured-tools.test.mjs` "当前数据集固定展示 12 条精华"用例仍与可选条目数隐性耦合——合格条目（⭐高频+📦常用）跌破 12 条时测试会翻红 | 断言改为 `featured.length === Math.min(12, eligible.length)` |
| R-7 | P4 | 同文件"非精华数量 = 总数 − 精华数量"是恒真断言，抓不到筛选逻辑的 bug | 换成"精华列表不含 status 为待试/偶尔的条目" |
