...
## Deferred Backlog（本轮不做）

### B3 日文件字段改为站点契约白名单

- **Why**：候选池内部字段与站点日文件契约目前靠
  `DAY_FILE_EXCLUDED_FIELDS` 黑名单相隔离；新增字段可能漏到日文件，
  误列字段又可能移除站点可见能力。2026-07-26 的 `score` 热度徽章
  回归说明，这类错误无法靠“测试全绿”自然发现。
- **Decision**：日文件不保留内部排序 `score`；站点若展示热度分，必须把
  `score` 视为可选字段或改用单独设计过的公开字段，不能让候选池内部评分
  重新泄漏到公开日文件。
- **What**：单独设计日文件序列化白名单，以
  `site/src/lib/twitter.ts` 的 `Tweet` 契约为真相源；`merge-day` 只输出
  白名单字段，池内部字段增删不应改变站点数据形状。
- **Acceptance**：新增契约测试，既验证 `score` 等池专用字段不会进入日文件，
  也验证日文件排除集不会移除站点必填字段；字段变更必须同步更新站点类型与该测试。
- **Boundary**：本轮先用跨语言契约断言堵住“剥掉站点必填字段”这类反转；
  白名单序列化作为后续单独改造。
