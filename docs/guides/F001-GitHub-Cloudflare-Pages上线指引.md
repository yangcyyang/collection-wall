...
忘记密码（可选）：WALL_RECOVERY_EMAIL（Cloudflare 里配置，示例 928590029cy@gmail.com，不要写进仓库） / RESEND_API_KEY / RESEND_FROM（可选；用该 Gmail 注册 Resend 后，默认 onboarding@resend.dev 即可寄到该邮箱）
```
